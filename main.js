...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.grandmaNames=['sumen', 'mr monk', 'azan'];
				...
				else
				{
					...
					if (version>=1)
					{
						...
						if (Game.ascensionMode!=1)
						{
							...
							if (Game.Has('Starter kitchen')) Game.Objects['Monkey'].free=5;
						}
		...
		Game.Reset=function(hard)
		{
			...
			if (!hard)
			{
				if (Game.ascensionMode!=1)
				{
					...
					if (Game.Has('Starter kitchen')) Game.Objects['Monkey'].getFree(5);
				}
			}
		...
		Game.computeLumpTimes=function()
		{
			...
			if (Game.Has('Sugar aging process')) Game.lumpRipeAge-=6000*Math.min(600,Game.Objects['Monkey'].amount);//capped at 600 grandmas
		...
		Game.getNewTicker=function(manual)//note : "manual" is true if the ticker was clicked, but may also be true on startup etc
		{
			...
			if (Game.TickerN%2==0 || loreProgress>14)
			{
				...
				{
					if (Game.Objects['Monkey'].amount>0) list.push('<q>'+choose(loc("Ticker (grandma)"))+'</q><sig>'+Game.Objects['Monkey'].single+'</sig>');
					
					if (!Game.prefs.notScary && Game.Objects['Monkey'].amount>=50) list.push('<q>'+choose(loc("Ticker (threatening grandma)"))+'</q><sig>'+Game.Objects['Monkey'].single+'</sig>');
					
					if (EN && Game.HasAchiev('Just wrong') && Math.random()<0.05) list.push(NEWS+'cookie manufacturer downsizes, sells own grandmother!');
					if (!Game.prefs.notScary && Game.HasAchiev('Just wrong') && Math.random()<0.4) list.push('<q>'+choose(loc("Ticker (angry grandma)"))+'</q><sig>'+Game.Objects['Monkey'].single+'</sig>');
					
					if (!Game.prefs.notScary && Game.Objects['Monkey'].amount>=1 && Game.pledges>0 && Game.elderWrath==0) list.push('<q>'+choose(loc("Ticker (grandmas return)"))+'</q><sig>'+Game.Objects['Monkey'].single+'</sig>');
			...
			if (EN && Game.season=='fools')
			{
				...
				if (Game.TickerN%2==0)
				{
					if (Game.Objects['Monkey'].amount>0) list.push(choose([
				...
				this.draw=function()
				{
					...
					*/
					...
					for (var i=0;i<len;i++)
					{
						...
						if (selected==i && this.name=='Monkey')
						{
		...
		new Game.Object('Monkey','monkey|monkeys|baked|Grandmas are [X] year older|Grandmas are [X] years older','A nice monkey to bake more cookies.',1,1,{pic:function(i){
			var list=['monkey'];
			...
			var add=0;
			if (Game.Has('One mind')) add+=Game.Objects['Monkey'].amount*0.02;
			if (Game.Has('Communal brainsweep')) add+=Game.Objects['Monkey'].amount*0.02;
		...
		new Game.Object('Farm','farm|farms|harvested|[X] more acre|[X] more acres','Grows cookie plants from cookie seeds.',3,2,{base:'farm',xV:8,yV:8,w:64,rows:2,x:0,y:16},500,function(me){
			...
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Monkey'].amount>0) Game.Unlock(this.grandma.name);
		});
		...
		Game.foolObjects={
			...
			'Monkey':{name:'Oven',desc:'A crucial element of baking cookies.',icon:1},
		...
		Game.GetTieredCpsMult=function(me)
		{
			...
			if (me.grandma && Game.Has(me.grandma.name)) mult*=(1+Game.Objects['Monkey'].amount*0.01*(1/(me.id-1)));
			return mult;
		}
		...
		order=200;
		Game.TieredUpgrade('Forwards from grandma','<q>RE:RE:thought you\'d get a kick out of this ;))</q>','Monkey',1);
		Game.TieredUpgrade('Steel-plated rolling pins','<q>Just what you kneaded.</q>','Monkey',2);
		Game.TieredUpgrade('Lubricated dentures','<q>squish</q>','Monkey',3);
		...
		order=200;Game.TieredUpgrade('Prune juice','<q>Gets me going.</q>','Monkey',4);
		...
		Game.GrandmaSynergy=function(name,desc,building)
		{
			...
			var grandmaNumber=loc("%1 monkey",LBeautify(building.id-1));
			desc=loc("%1 are <b>twice</b> as efficient.",cap(Game.Objects['Monkey'].plural))+' '+loc("%1 gain <b>+%2%</b> CpS per %3.",[cap(building.plural),1,grandmaNumber])+'<q>'+desc+'</q>';
			
			var upgrade=new Game.Upgrade(name,desc,building.basePrice*Game.Tiers[2].price,[10,9],function(){Game.Objects['Monkey'].redraw();});
		...
		new Game.Upgrade('Ritual rolling pins',loc("%1 are <b>twice</b> as efficient.",cap(Game.Objects['Monkey'].plural))+'<q>The result of years of scientific research!</q>',4000000000000000,[2,9],function(){Game.SetResearch('Underworld ovens');});Game.last.pool='tech';
		...
		order=200;Game.TieredUpgrade('Double-thick glasses','<q>Oh... so THAT\'s what I\'ve been baking.</q>','Monkey',5);
		...
		order=200;Game.TieredUpgrade('Aging agents','<q>Counter-intuitively, grandmas have the uncanny ability to become more powerful the older they get.</q>','Monkey',6);
		...
		order=200;Game.TieredUpgrade('Xtreme walkers','<q>Complete with flame decals and a little horn that goes "toot".</q>','Monkey',7);
		...
		order=200;Game.TieredUpgrade('The Unbridling','<q>It might be a classic tale of bad parenting, but let\'s see where grandma is going with this.</q>','Monkey',8);
		...
		order=200;Game.TieredUpgrade('Reverse dementia','<q>Extremely unsettling, and somehow even worse than the regular kind.</q>','Monkey',9);
		...
		order=200;Game.TieredUpgrade('Timeproof hair dyes','<q>Why do they always have those strange wispy pink dos? What do they know about candy floss that we don\'t?</q>','Monkey',10);
		...
		order=5000;
		Game.SynergyUpgrade('Script grannies','<q>Armies of energy drink-fueled grandmas ready to hack into the cyberspace for renegade e-cookies.</q>','Javascript console','Monkey','synergy1');
		...
		Game.TieredUpgrade('Fortune #002','<q>A wrinkle is a crack in a mundane facade.</q>','Monkey','fortune');
		...
		order=200;Game.TieredUpgrade('Generation degeneration','<q>Genetic testing shows that most of your grandmas are infected with a strange degenerative disease that only seems to further their powers; the more time passes, the older they get. This should concern you.</q>','Monkey',12);
		...
		Game.NewUnshackleBuilding({building:'Monkey',q:'Never too old.'});
		...
		order=200;Game.TieredUpgrade('Foam-tipped canes','<q>Perhaps the result of prolonged service, your grandmas have developed all kinds of odd and aggressive hierarchies among themselves; these will help them not hurt each other as bad during their endless turf wars.</q>','Monkey',15);
		...
		order=1100;
		Game.TieredAchievement('Grandma\'s cookies','','Monkey',1);
		Game.TieredAchievement('Sloppy kisses','','Monkey',2);
		Game.TieredAchievement('Retirement home','','Monkey',3);
		...
		order=1100;
		Game.TieredAchievement('Friend of the ancients','','Monkey',4);
		Game.TieredAchievement('Ruler of the ancients','','Monkey',5);
		...
		order=1120;
		Game.ProductionAchievement('Gushing grannies','Monkey',1,0,6);
		order=1220;
		...
		order=1100;Game.TieredAchievement('The old never bothered me anyway','','Monkey',6);
		...
		order=1120;
		Game.ProductionAchievement('Panic at the bingo','Monkey',2,0,6);
		order=1220;
		...
		order=1100;
		Game.TieredAchievement('The agemaster','','Monkey',7);
		Game.TieredAchievement('To oldly go','','Monkey',8);
		...
		order=1120;
		Game.ProductionAchievement('Frantiquities','Monkey',3,0,6);
		order=1220;
		...
		order=1120;
		new Game.Achievement('Methuselah','',[1,26]);Game.Objects['Monkey'].levelAchiev10=Game.last;
		order=1220;
		...
		order=1100;
		Game.TieredAchievement('Aged well','','Monkey',9);
		Game.TieredAchievement('101st birthday','','Monkey',10);
		Game.TieredAchievement('But wait \'til you get older','','Monkey',11);
		...
		order=1100;Game.TieredAchievement('Defense of the ancients','','Monkey',12);
		...
		order=1100;Game.TieredAchievement('Okay boomer','','Monkey',13);
		...
		order=1100;Game.TieredAchievement('They moistly come at night','','Monkey',14);
		...
		order=1100;Game.TieredAchievement('And now you\'re even older','','Monkey',15);
		...
		*/
		...
		{
			if (Game.Has('Elder Covenant') || Game.Objects['Monkey'].amount==0) Game.elderWrath=0;
	...
	Game.Logic=function()
	{
		...
		if (!Game.OnAscend && Game.AscendTimer==0)
		{
			...
			if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
			{
				...
				if (Game.Objects['Monkey'].amount>=6 && !Game.Has('Bingo center/Research facility') && Game.HasAchiev('Elder')) Game.Unlock('Bingo center/Research facility');
				...
				if (Game.Objects['Cursor'].amount+Game.Objects['Monkey'].amount>=777) Game.Win('The elder scrolls');
		...
		{
			var title='Banana Clicker';
			if (Game.season=='fools') title='Cookie Baker';
			document.title=(Game.OnAscend?(EN?'Ascending! ':(loc("Ascending")+' | ')):'')+loc("%1 banana",LBeautify(Game.cookies))+' - '+title;
		}
		if (Game.T%15==0)
		{
	...
	Game.Draw=function()
	{
		...
		if (!Game.OnAscend)
		{
			...
			str=loc("%1 banana",{n:Math.round(Game.cookiesd),b:str});
					...
					var launch=function(){
						...
						else
						{
							console.log('[=== '+choose([
								...
								'Well well well look at who is at the conole',
							])+' ===]');
